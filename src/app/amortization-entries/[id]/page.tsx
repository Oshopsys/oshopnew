...
import { AMORTIZATION_ENTRIES, INTANGIBLE_ASSETS } from "@/lib/mock-data";
import { DeleteDialog } from "@/components/delete-dialog";

export default function AmortizationEntryDetailsPage() {
            ...
            <div className="max-w-4xl mx-auto space-y-6">
                <Card>
                    <CardContent className="p-6 text-center">
                        <p className="text-muted-foreground">قيد الإطفاء غير موجود</p>
                        <Button className="mt-4" onClick={() => router.push("/amortization-entries")}>
                            العودة للقائمة
                        </Button>
                    </CardContent>
                </Card>
    ...
    const handleDelete = () => {
        ...
        const index = AMORTIZATION_ENTRIES.findIndex((e: any) => e.id === id);
        if (index > -1) {
            AMORTIZATION_ENTRIES.splice(index, 1);
        }
        ...
        <>
            <div className="max-w-4xl mx-auto space-y-6">
                <div className="flex items-center justify-between">
                    <h1 className="text-2xl font-bold">تفاصيل قيد الإطفاء</h1>
                    <div className="flex gap-2">
                        <Button variant="outline" onClick={() => router.push("/amortization-entries")}>
                            <ArrowLeft className="ml-2 h-4 w-4" />
                            رجوع
                        </Button>
                        <Button variant="outline" asChild>
                            <Link href={`/amortization-entries/${id}/edit`}>
                ...
                <Card>
                    <CardHeader>
                        <CardTitle>تفاصيل الأصول المطفأة</CardTitle>
                    </CardHeader>
                    <CardContent>
                        <Table>
                            <TableHeader>
                                <TableRow>
                                    ...
                                    <TableHead className="text-right">الأصل غير الملموس</TableHead>
                                    <TableHead className="text-right">مبلغ الإطفاء</TableHead>
                                </TableRow>
                            </TableHeader>
                            <TableBody>
                                {lines.map((item: any, index: number) => {
                                    const asset = INTANGIBLE_ASSETS.find(a => a.id === item.assetId);
                                    return (
                                        <TableRow key={index}>
            ...
            <DeleteDialog
                ...
                title="تأكيد الحذف"
                description="هل أنت متأكد من حذف قيد الإطفاء هذا؟"
