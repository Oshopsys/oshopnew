...
export default function PurchaseInvoiceForm({
    ...
    const defaultValues: InvoiceFormValues = initialData
        ? {
            ...
            supplierId: initialData.partner_id || "",
            billingAddress: initialData.notes || "",
    ...
    async function onSubmit(data: InvoiceFormValues) {
        try {
            if (mode === "edit" && invoiceId) {
                // TODO: Update logic
                // toast({ title: "Working on it", description: "Edit functionality coming soon" });
                ...
                const invoiceData = {
                    ...
                    // reference: data.reference // Removed as column doesn't exist
                };

                const invoiceLines = data.lines.map(line => ({
                    item_id: line.itemId || null, // Allow null for free text? SQL allows?
                ...
                console.log('Purchase invoice creation result:', result);
                ...
                if (result && (result.success || result.invoice_id)) {
                    ...
                    alert("❌ فشل إنشاء الفاتورة: " + (result?.error || 'خطأ غير معروف'));
                }
            }
        } catch (error: any) {
            console.error('Error in purchase invoice creation:', error);
    ...
    const handleItemSelect = (index: number, itemId: string) => {
        ...
        if (selectedItem) {
            ...
            // For purchase, usually we enter the cost manually, but we can default to 0
            form.setValue(`lines.${index}.unitPrice`, 0);
        }
    };
            ...
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                <div className="flex items-center justify-between">
                    <h1 className="text-2xl font-bold">
                        {mode === "edit" ? "تعديل فاتورة شراء" : "فاتورة شراء جديدة"}
                    </h1>
                                    ...
                                    <FormControl>
                                        <div className="flex gap-2">
                                            <div className="flex items-center space-x-2">
                                                <Input placeholder="رقم فاتورة المورد" {...field} />
                                            </div>
                                        </div>
                                    </FormControl>
                                    <FormMessage />
